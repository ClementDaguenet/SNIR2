...
void UIServerTcp::on_pushButton_Lancer_clicked()
{
  if(socketServer.listen(QHostAddress::Any,ui->spinBox_Port->value()))
    ...
    {
      ui->textEdit_Etat->append("Launch failed : " + socketServer.errorString());
    }
}
...
void UIServerTcp::onQTcpServerNewConnection()
{
  ...
  ui->textEdit_Etat->append(socketClient->peerAddress().toString() + " connected");
  listeSocketClient.append(socketClient);
}

void UIServerTcp::onQTcpSocketConnected()
{
  QTcpSocket *socketClient;
  socketClient = qobject_cast<QTcpSocket*>(sender());
  ui->textEdit_Messages->append(socketClient->peerAddress().toString() + " connected");
...
void UIServerTcp::onQTcpSocketDisconnected()
{
  QTcpSocket *socketClient;
  socketClient = qobject_cast<QTcpSocket*>(sender());
  ui->textEdit_Messages->append(socketClient->peerName() + " disconnected");
  // désactiver boutons connexion port
  ui->pushButton_Lancer->setDisabled(false);
  ui->spinBox_Port->setDisabled(false);
  // retirer les gestions d'event de la socket
  disconnect(socketClient,nullptr,this,nullptr);
  // supression de la liste
  listeSocketClient.removeOne(socketClient);
  // destruction de socketClient
...
void UIServerTcp::onQTcpSocketReadyRead()
{
  ...
  ui->textEdit_Messages->append(socketClient->peerAddress()/*.toIPv4Address()*/.toString() + " " + data);
